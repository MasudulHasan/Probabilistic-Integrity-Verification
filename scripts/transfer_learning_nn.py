...
for model in selected_models:
    ...
    while(month_to_be_considerate <= 9):
        ...
        out_file = out_file = open("../transfer_learning_result/"+str(month_to_be_considerate)+"/"+model+".txt","a+")
        for test_model in selected_models:
            hdd_test = pd.read_csv("../month_wise_file/"+str(year_to_cosider)+"/"+str(prev_month)+'/'+str(test_model)+'.csv', header=None)
