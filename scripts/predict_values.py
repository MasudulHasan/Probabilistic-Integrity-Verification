...
# out_file = open("model_test_result_"+disk_model_name+".txt","a+")
while(month_to_be_considerate<=9):
    df_list = []
    year_to_cosider=2019
    prev_month = month_to_be_considerate-month_range
    if month_to_be_considerate<(month_range+1):
        prev_month=(month_to_be_considerate+12)-month_range
        year_to_cosider-=1
    
    for iter in range(month_range):
    ...
    for day in range(1, 32):
        try:
            ...
            if correctDate == True:
                ...
                output_file = open("../predicted_result_new/"+str(date)+".csv", "a+")
