...
def runner(number_of_max_disk, df, date,year,month,day):
    ...
    for iter_n in range(100):
        try:
            file_size_list=[1024, 10240, 102400, 1048576, 10485760, 52428800, 104857600, 262144000, 1073741824, 5368709120]
            total_file_size=6873529344
            output_str += "\n\nfile size "+ str(file_size_list)+"\n"
            total_disk_number = 10
            # disk_number_list=[]
            block_size_list=[1024, 10240, 102400, 1048576, 10485760, 52428800, 104857600, 262144000, 1073741824, 5368709120]
            shape = df.shape
            import random
...
#                print("1 ",time.time()-start)
            ...
            t=[calibration(x, 41497248, 5390, 10780, 5390) for x in preds]
            prob_list=[]
            divisor = 16*pow(10,9)
            ...
            selected_disk = get_best_disk_greedy_new(prob_list, n, number_of_error,total_file_size,10)
            selected_disk_brute = get_best_disk(prob_list, n, number_of_error,total_file_size,10,block_size_list)
            ...
            if selected_disk is not None:
                ...
                output_str+="brute I/O saved = "+str(total_io)+" "+str(saved_io)+" "+str(perc)+"\n"
...
for part in range(2):
    stripe_size = 50
    if start_day==1:
        output_file = open("../result_log/bruet_force/"+str(disk_model_name)+"_"+str(month)+"_first_combined_greedy.txt","a+")
    else:
        output_file = open("../result_log/bruet_force/"+str(disk_model_name)+"_"+str(month)+"_second_combined_greedy.txt","a+")
