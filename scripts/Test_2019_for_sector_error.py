...
out_file = open("../result_2019/result_"+str(day)+"_for_sectorerror_without_2019_data.txt","a+")
# print(folder_list)
...
for x in folder_list:
  try:
    ...
    if model_name in selected_models:
      ...
      test_hdd = pd.read_csv("../test_file_for_2019/1/"+model_name+".csv", header=None)
      test_hdd = test_hdd.drop(test_hdd.columns[10], axis=1)
      test_hdd = test_hdd.drop(test_hdd.columns[15], axis=1)
      test_hdd = test_hdd.drop(test_hdd.columns[13], axis=1)
      test_hdd = test_hdd.dropna()
      
#      test_hdd_pos = test_hdd.loc[test_hdd[test_hdd.columns[13]] == 1]
#      test_hdd_neg = test_hdd[test_hdd[test_hdd.columns[13]] == 0]
      
#      pos_train, pos_test = train_test_split(test_hdd_pos, test_size=0.2)
#      neg_train, neg_test = train_test_split(test_hdd_neg, test_size=0.2)
      
#      del test_hdd
#      gc.collect()


#      train_merged = [hdd_pos, hdd_neg, pos_train, neg_train]
      train_merged = [hdd_pos, hdd_neg]
