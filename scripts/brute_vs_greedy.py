...
def calculate_error_prob(prob_list):
    ...
    final_prob =0
    for x in range(len(prob_list)):
        final_prob+=(prob_list[x]/(1-prob_list[x]))
    return zero_error_prob*final_prob
...
def get_best_disk(prob_list, n, number_of_error,file_size,factor,block_size_list,alpha):
    ...
    for x in allsubsets(n):
        ...
        prob=calculate_error_prob(unslected_prob_list)
    ...
    normalised_block_size_list = stats.zscore(selected_data_size_list).tolist()

    for x in range(len(normalized_selected_prob_list)):
      cost = (alpha*normalised_block_size_list[x])+(1-alpha)*normalized_selected_prob_list[x]
...
def get_best_disk_greedy_new(prob_list, n, number_of_error,file_size_list,factor,alpha, file_size):
    ...
    for i in range(len(prob_list)):
        ...
        prob = calculate_error_prob(new_lst)
        selected_prob_list.append(prob)
    ...
    for i in range(len(normalised_file_size_list)):
        ...
        cost_list.append(p1)
    prob=calculate_error_prob(new_prob_list)
    val = prob/(file_size*8)
    if val<=threshold or ((val-threshold)/threshold)<=.1:
        return []
    ...
    while(True):
        ...
        prob=calculate_error_prob(res_prob_list)
        val = prob/(file_size*8)
        if val<=threshold or ((val-threshold)/threshold)<=.1:
            selected_list=[]
...
for part in range(2):
    if start_day==1:
        output_file = open("../result_log/analysis_with_alpha/"+str(disk_model_name)+"_"+str(month)+"_first_combined.txt","a+")
    else:
        output_file = open("../result_log/analysis_with_alpha/"+str(disk_model_name)+"_"+str(month)+"_second_combined.txt","a+")
