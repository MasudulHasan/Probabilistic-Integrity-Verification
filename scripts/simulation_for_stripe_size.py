...
def runner(number_of_max_disk, df, date,year,month,day):
    ...
    for iter_n in range(100):
        try:
            ...
            if s>.9:
                file_size = np.random.poisson(lam=1.165580e+09)
            else:
            ...
            stripe_index = randint(0, 5)
            stripe_number = stripes[stripe_index]
            # disk_number = min (number_of_max_disk, int(file_size/(128*1024))+1)
            disk_number = number_of_max_disk*stripe_number
            shape = df.shape
...
#                print("1 ",time.time()-start)
            ...
            selected_disk = get_best_disk_greedy_new(prob_list, n, number_of_error,file_size,10)
...
#                print("2 ",time.time()-start, number_of_error)
            ...
            if selected_disk is not None:
                ...
                output_str+="stripe number : "+str(stripe_number)+"\n"
...
for part in range(2):
    stripe_size = 50
    if start_day==1:
        output_file = open("../result_log/analysis_with_stripe/"+str(disk_model_name)+"_"+str(month)+"_first_combined.txt","a+")
    else:
        output_file = open("../result_log/analysis_with_stripe/"+str(disk_model_name)+"_"+str(month)+"_second_combined.txt","a+")
